        ...
        return this.each(function() {
            ...
            $(document).on("click", '#' + class_name + ' span.highlight', function() {
                ...
                if ($('.back_btn').length == 0) {
                    $('body').prepend('<img src="' + wm_options.images_url + 'back.png" class="back_btn" />');
                }
                call_structure();
            ...
            if (add_option) { // ++add
                $(document).on("click", '#' + class_name + ' span.add_action', function() {
                    ...
                    var data = {
						action: 'wm_node_addform',
						tr_parent_id: $(this).closest("div").attr("id")
					};
					showProcessStatusMessage(true, 'waiting', 'Retrievng a from..');
                    $.ajax({
                        type: 'POST',
                        ...
                        success: function(data) {
                            ...
                            if (_this.closest('div').children('form.add_data').length == 0) {
							   ...
							   _this.closest('div').children('form').children('input[type="text"]').each(function(){
                    ...
                    $(document).on("click", "input.submit", function(event) {
                        ...
                        var data = "action=wm_add_node&parentid=" + parentid + "&";
                        data += _addthis.closest('form').serialize();
                        _addthis.closest("li").before('<img src="' + wm_options.images_url + 'load.gif" class="load" />');
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
								...
								var title = '';
								if(data.success){
									showProcessResultMessage('success', 'Node is saved successfully!');
									img_url = data.tr_thumbnail_url;
									title = data.tr_title;
								...
								/* content */
								var content = [];
								if(wm_options.mp_enable_thumbnail){
									content[content.length] = '<img src="' + img_url + '" width="' + wm_options.mp_thumbnail_size +
									'" height="' + wm_options.mp_thumbnail_size + '" class="wm-thumbnail-img ' + wm_options.mp_thumbnail_shape + '" />';
									content[content.length] = '<div class="wm-cleaner"></div>';
								}
								content[content.length] = "<span class='first_name'>" + title + "</span>"; // _addthis.closest('form').find('input.first_name').val()
								
								if(wm_options.mp_thumbnail_place == 'under'){
                                ...
                                ajax_add_id = data;
                                var html_value = '<li>' + vertical_line_text + horizontal_line_text + '<div id="' + ajax_add_id + '">' + highlight_text + add_action_text + delete_action_text + edit_action_text + content.join("") + '</div></li>';
            ...
            if (edit_option) { // ++edit
                $(document).on("click", '#' + class_name + ' span.edit_action', function() {
                    ...
                    $(document).on("click", "input.edit", function(event) {
                        var _editthis = $(this);
                        event.preventDefault();
                        var data = "action=wm_edit_node&id=" + _editthis.closest('div').attr('id') + "&";
                        data += _editthis.closest('form').serialize();
                        _editthis.closest("li").before('<img src="' + wm_options.images_url + 'load.gif" class="load" />');
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (_editthis.closest('form').find('input#hide:checked').length > 0) {
								...
								content[content.length] = "<span class='first_name'>" + title + "</span>"; // _editthis.closest('form').find('input.first_name').val()
            ...
            if (delete_option) { // ++delete
                $(document).on("click", '#' + class_name + ' span.delete_action', function() {
                    ...
                    if ( isEmpty( $(this).closest('div').data('parent-id') ) ) { // $(this).closest('div').attr('id') == null
                        alert("You can't delete root node");
                    } else {
                        if (confirm_message) {
                            ...
                            var data = {
								action: 'wm_node_delete',
								id: ajax_delete_id
							};
                            $(this).closest("li").before('<img src="' + wm_options.images_url + 'load.gif" class="load" />');
							
							showProcessStatusMessage(true, 'waiting', 'Deleting node..');
                            $.ajax({
                                type: 'POST',
                                url: wm_options.admin_url + 'admin-ajax.php',
                                data: data,
                                success: function(data) {
									showProcessResultMessage('success', 'Node is deleted successfully!');
                                    $("img.load").remove();
                    ...
                    $('#' + class_name + ' li > div').droppable({
                        ...
                        drop: function(event, ui) {
                            ...
                            if ($(this).hasClass('check_div')) {
                                ...
                                var data = {
									action: 'wm_drag_node',
									tr_id: $(ui.draggable[0]).attr('id'),
									tr_parent_id: $(this).attr('id')
								}
                                ...
                                $.ajax({
                                    ...
                                    data: data,
                                    success: function(data) {
										showProcessResultMessage('success', 'Node is moved successfully!');
										setTimeout(function(){
											wm_setContainerDimentions();
										}, 4000);
                                    }
