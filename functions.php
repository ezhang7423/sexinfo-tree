...
function wm_admin_styles_scripts()
{
    ...
    if (preg_match('/^.*wm_website_maps$/', $current_screen->id) || preg_match('/^.*wm_edit_website_map$/', $current_screen->id)) {
        ...
        wp_enqueue_style('wm-css', plugins_url('css/stylecss.css', WM_PLUGIN_MAIN_FILE));
        ...
        wp_enqueue_script('wm-admin-js', plugins_url('js/adminjs.js', WM_PLUGIN_MAIN_FILE));
...
{
    add_menu_page('WP Tree', 'WP Tree', 'manage_options', 'wm_website_maps', 'wm_create_maps_page', 'dashicons-networking', 62);
 ...
 * @return array
 */
function wm_read_map_tree_by_parent($mp_id, $tr_parent_id = NULL, $posts_list = NULL)
{
    global $wpdb;
    $mp_id = (int)$mp_id;
    $sql = "SELECT tr_id, tr_parent_id, tr_title, tr_link, tr_active_link, tr_hide, tr_thumbnail_url, tr_type 
			FROM {$wpdb->prefix}wm_trees 
			WHERE mp_id = %d";
...
function wm_get_vertical_tree_nodes()
{
    ...
    if (isset($map['mp_id'])) {
        if ('#' === $parent) {
            $map_nodes = wm_read_map_tree_by_parent($mp_id, NULL);
        } else {
            $posts_list = NULL;
            ...
            $map_nodes = wm_read_map_tree_by_parent($mp_id, $parent, $posts_list);
        }
        ...
        if ($html) {
            $out = '<ul>';
            foreach ($nodes as $n) {
                $text = ($map['mp_enable_link'] && !empty($n['link']) && !$control_panel) ? '<a href="' . $n['link'] . '" target="_parent" onclick="wm_toLink(this);" >
														<span>' . $n['text'] . '</span>
													</a>' :
                    '<span>' . $n['text'] . '</span>';
...
function wm_get_icon($tr_type)
{
    switch ($tr_type) {
        ...
        case 'post_type':
            return plugins_url('images/folder_icon_22.png', WM_PLUGIN_MAIN_FILE);
            break;
        ...
        case 'term':
            return plugins_url('images/category_icon_22.png', WM_PLUGIN_MAIN_FILE);
            break;
    }
}
...
function wm_enqueue_for_frame()
{
    ...
    <link rel="stylesheet" type="text/css" href="<?php echo plugins_url('css/stylecss.css', WM_PLUGIN_MAIN_FILE) ?>"/>
    <script type="text/javascript"
...
function wm_wptree_insert_records_postdata($post_id)
{
    ...
    if (isset($_POST['post_status']) && $_POST['post_status'] == 'publish' && $data_check != 'added' && $modify_date == $date_create) {
        ...
        $get_results = $wpdb->get_results("select tree.tr_id,tree.mp_id from $tbl_tree as tree left join $tbl_maps as map on map.mp_id = tree.mp_id where tree.tr_type = 'post_type' and tree.tr_title = 'Pages' AND map.mp_type='vertical' ");
...
add_action('save_post', 'wm_wptree_insert_records_postdata');
